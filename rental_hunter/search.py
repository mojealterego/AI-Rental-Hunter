...
Search Polish portals and property sites. Prefer individual listing pages. Apply these hard constraints when selecting candidates: the final monthly recurring cost must be no greater than the requested max_monthly_total; exclude agencies when no_agency=true; respect room, area, furnishing, district and contract constraints. Return up to {max(criteria.max_results * 3, 50)} distinct candidate offers so the application can strictly filter them to the requested final count. Never treat the advertised rent alone as the monthly total when additional recurring fees are stated.
JSON schema:
